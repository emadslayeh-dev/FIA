    ...
    <>
      <div
        ...
        <div className='offcanvas-header border-bottom'>
          <Link href='/'>
            <Image alt='Header Logo' width='143' height='60' src='/images/HEADER-LOGO-FIA.png' />
          </Link>
          <button ref={crossRef} type='button' className='btn-close' data-bs-dismiss='offcanvas' aria-label='Close' />
        </div>
                ...
                {navigation.map((item, i) =>
                  item?.children ? (
                    <SubMenu
                      key={i}
                      label={item.name}
                      className={isActiveNavigation(path, item) ? "ui-mobile-active" : ""}>
                      {item.children.map((item2, i2) =>
                        item2?.children ? (
                          <SubMenu
                            key={i2}
                            label={item2.name}
                            className={isActiveNavigation(path, item2) ? "ui-mobile-active" : ""}>
                            {item2.children.map((item3, i3) => (
                              <MenuItem
                                key={i3}
                                component={<Link href={item3.path} />}
                                className={
                                  item3.path === path || item3.path === path.replace(/\/\d+$/, "")
                                    ? "ui-mobile-active"
                                    : ""
                                }>
