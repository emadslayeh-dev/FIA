...
export default function CompanyReviewList({ companyId, reloadTrigger = 0, onChanged }) {
  ...
  useEffect(() => {
    if (!companyId) return;
    setLoading(true);
    fetch(`/api/company-reviews?companyId=${companyId}`)
      .then((r) => r.json())
      .then((d) => {
  ...
  function submitEdit(e) {
    ...
    fetch("/api/company-reviews", {
      ...
      .then((d) => {
        if (d?.ok && d?.data) {
          setReviews((prev) => prev.map((r) => (r.id === editingId ? d.data : r)));
          cancelEdit();
          onChanged && onChanged();
        }
      })
  ...
  function deleteReview(id) {
    ...
    fetch(`/api/company-reviews?reviewId=${id}`, { method: "DELETE" })
      .then((r) => r.json())
      .then((d) => {
        if (d?.ok) {
  ...
  return (
    <div className='product_single_content mb30' dir='rtl'>
      <div className='mbp_pagination_comments'>
        <div className='total_review mb20 mt10'>
          <h4>{average ? `${reviews.length} ביקורות על החברה` : "אין ביקורות על החברה"}</h4>
        </div>
