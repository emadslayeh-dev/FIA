...
export default function TabSection1() {
  ...
  return (
    <>
      <section className={`categories_list_section overflow-hidden ${path === "/" ? "bgc-thm5" : ""}`}>
        <div className='container'>
          <div className='row'>
            <div className='col-lg-12'>
              <div className='listings_category_nav_list_menu \'>
                <ul className='mb2 d-flex ps-0'>
                  {categories.map((item, index) => (
                    <li key={index}>
                      <a onClick={() => setCurrentTab(item)} className={getCurrentTab == item ? "active" : ""}>
                        {item}
                      </a>
                    </li>
                  ))}
